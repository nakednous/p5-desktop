...
public void setup() {
  ...
  //stop condition: noLoop interpolation needed
...
public void draw() {
  ...
  //stop condition
  if(kfi.interpolationTime()>=kfi.keyFrameTime(index))
    kfi.stopInterpolation();
  //debug:
  println(kfi.interpolationTime());
...
public void keyPressed() {
  //stop condition: index update
  if(key==' ') {
    kfi.startInterpolation();
    if(index + 1 <= kfi.numberOfKeyFrames()-1)
      index++;
    else
      index = 1;
  }
